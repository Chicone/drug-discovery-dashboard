...
function Docking() {
	const [residueView, setResidueView] =
		useState("distance"); // "distance" | "sequence"
	const [
		selectedRunId,
		setSelectedRunId,
	] = useState(null);
	...
	const [ligandFile, setLigandFile] =
		useState(null);
	const [pdbText, setPdbText] =
		useState("");
	...
	const [boxFile, setBoxFile] =
		useState(null);
	const [mdJobId, setMdJobId] =
		useState(null);
	const [mdPreset, setMdPreset] =
		useState("cg_popc_50ns");
	const [mdLaunching, setMdLaunching] =
		useState(false);
	const [mdError, setMdError] =
		useState(null);

	// 🔒 Persisted lock state for docking runs
	const [lockedRuns, setLockedRuns] =
		useState(() => {
			try {
				return JSON.parse(
	...
	const [dockParams, setDockParams] =
		useState({
			num_modes: 10,
			exhaustiveness: 8,
			cpu: 0,
	...
	const applyBoxFromJsonText = (
		...
		if (
			...
			throw new Error(
				"Invalid box JSON: expected box_center and box_size arrays of length 3."
			);
		}
	...
	const handleDocking = async () => {
		if (!receptorFile || !ligandFile) {
			setError(
				"Please upload both receptor and ligand files."
			);
			return;
		}
		...
		formData.append(
			"num_modes",
			dockParams.num_modes
		);
		formData.append(
			"exhaustiveness",
			dockParams.exhaustiveness
		);
		formData.append(
			"cpu",
			dockParams.cpu
		...
		try {
			...
			if (
				...
				setError(
					"Docking completed but returned no poses."
				);
				return;
			}
	...
	async function runMdFromDocking() {
		...
		if (!selectedRunId) {
			setMdError(
				"No docking run selected. Run docking or click a run in history."
			);
			return;
		}
	...
	const loadPoseAnalysis = async (
		...
		try {
			...
			console.error(
				"Failed to load pose analysis:",
				e
			);
		}
	};

	const groupResiduesIntoRanges = (
		residuesSeq
	) => {
	...
	const selectedMode =
		...
		poses?.[selectedPoseIdx]
			? Number(
					poses[selectedPoseIdx].mode
			  )
			: null;

	function toggleRunLock(runId) {
		setLockedRuns((prev) => {
			const updated = {
				...prev,
				[runId]: !prev[runId],
			...
			>
				...
				>
					...
					>
						...
						<Box sx={{ mb: 1.5 }}>
							<Tooltip
								title="Load, if available, a binding-site box from a box.json file to auto-fill the docking box fields"
								arrow
							>
								<Button
									variant="contained"
									...
									<Button
										...
										onClick={async () => {
											try {
												...
												if (!res.ok) {
													alert(
														"Could not download complex PDB"
													);
													return;
												}
											...
											(run) => (
												...
												>
													...
													>
														...
														{typeof run.best_score ===
															"number" && (
															<>
																{" "}
																— best{" "}
																{run.best_score.toFixed(
																	2
																)}
															</>
														)}
